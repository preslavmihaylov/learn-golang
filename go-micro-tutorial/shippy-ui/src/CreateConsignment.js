...
class CreateConsignment extends React.Component {
  ...
  componentWillMount() {
    fetch(`http://localhost:8080/shippy-consignment-service/shipping-service/get-consignments`)
    .then(req => req.json())
    .then((res) => {
      this.setState({
  ...
  create = () => {
    ...
    fetch(`http://localhost:8080/shippy-consignment-service/shipping-service/create-consignment`)
    .then((res) => res.json())
    .then((res) => {
      this.setState({
        created: res.created,
